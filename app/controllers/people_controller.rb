# -*- encoding : utf-8 -*-
class PeopleController < ApplicationController
  # GET /people
  # GET /people.json

  def index
    @people = Person.all

    respond_to do |format|
      format.html # index.html.erb
      format.json { render json: @people }
    end
  end

  # GET /people/1
  # GET /people/1.json
  def show
    @person = Person.find(params[:id])

    respond_to do |format|
      format.html # show.html.erb
      format.json { render json: @person }
    end
  end

  # GET /people/new
  # GET /people/new.json
  def new
    @person = Person.new
    @person.build_employee

    respond_to do |format|
      format.html # new.html.erb
      format.json { render json: @person }
    end
  end

  # GET /people/1/edit
  def edit
    @person = Person.find(params[:id])
  end

  # POST /people
  # POST /people.json
  def create
    @person = Person.new(params[:person])

    respond_to do |format|
      if @person.save
        format.html { redirect_to @person, notice: 'Person was successfully created.' }
        format.json { render json: @person, status: :created, location: @person }
      else
        format.html { render action: "new" }
        format.json { render json: @person.errors, status: :unprocessable_entity }
      end
    end
  end

  # PUT /people/1
  # PUT /people/1.json
  def update
    @person = Person.find(params[:id])

    respond_to do |format|
      if @person.update_attributes(params[:person])
        format.html { redirect_to @person, notice: 'Person was successfully updated.' }
        format.json { head :no_content }
      else
        format.html { render action: "edit" }
        format.json { render json: @person.errors, status: :unprocessable_entity }
      end
    end
  end

  # DELETE /people/1
  # DELETE /people/1.json
  def destroy
    @person = Person.find(params[:id])
    @person.destroy

    respond_to do |format|
        format.html { redirect_to people_url }
      format.json { head :no_content }
    end
  end

  def sync
    @abanits = Abanit.where("bempleado = ?", 'T').find(:all, 
                            :select => ['init', 'ntercero'])
    @syn_data = {}
    people = []
    @c = 0
    @abanits.each do |person|
      if Person.where("id_person = ?", person.init).empty?
        full_name = splitname(firebird_encoding(person.ntercero).split)
        person_new = Person.create(:name => full_name[:name], :first_surname =>
                      full_name[:first_surname], :second_surname => full_name[:second_surname], 
                      :id_person => person.init)
        if person_new.save
          people << person_new
          @c += 1
        else
          person_new.errors.each do |error|
            Rails.logger.error "Error Creating Person: #{person.init}, 
                                Description: #{error}"
          end
        end        
      end
    end
    @syn_data[:people] = people
    @syn_data[:notice] = "#{t('helpers.titles.sync').capitalize}: #{@c}"
    respond_to do |format|
        format.json { render json: @syn_data }
      end
  end

  def splitname(splitname)
    full_name = {}
    if splitname.count < 4
      full_name[:name] = splitname.first
      full_name[:first_surname] = splitname[1]
      full_name[:second_surname] = splitname[2]
    elsif splitname.count > 3
      n = splitname.count - 2
      full_name[:name] = splitname[0, n].join(" ")
      full_name[:first_surname] = splitname[n]
      full_name[:second_surname] = splitname.last        
    end
    full_name
  end
end
